...
app.put(
  ...
  async (req: express.Request & { user?: JwtPayload }, res: express.Response) => {
    ...
    const { id } = req.params
    const { tipoVenta, tipoEntrega, metodoPago, observacion, descuentoTotal } = req.body as {
      tipoVenta?: string
      tipoEntrega?: string
      metodoPago?: string
      observacion?: string
...
app.post('/public/pedidos', async (req, res) => {
  ...
  try {
    ...
    const subtotal = carrito.reduce((acc: number, item: any) => acc + (item.cantidad * item.precioUnitario), 0)
