...
app.get('/reportes/stock-bajo', authMiddleware, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'No autorizado' });
...
app.get('/reportes/stock-bajo/excel', authMiddleware, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'No autorizado' });
    const umbral = Math.min(Math.max(Number(req.query.umbral) || UMBRAL_STOCK_BAJO_DEFAULT, 0), 100);
    try {
...
app.get('/reportes/apartados-por-vencer', authMiddleware, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'No autorizado' });
...
app.get('/reportes/apartados-por-vencer/excel', authMiddleware, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'No autorizado' });
    const dias = Math.min(Math.max(Number(req.query.dias) || 30, 1), 90);
    try {
...
app.get('/reportes/productos-mas-vendidos', authMiddleware, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'No autorizado' });
...
app.get('/reportes/productos-mas-vendidos/excel', authMiddleware, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'No autorizado' });
    const limit = Math.min(Number(req.query.limit) || 100, 500);
    try {
...
app.get('/ventas/export/excel', authMiddleware, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'No autorizado' });
    const { desde, hasta } = req.query;
    let desdeDate = null;
...
app.get('/ventas/export/pdf', authMiddleware, async (req, res) => {
    if (!req.user)
        return res.status(401).json({ message: 'No autorizado' });
    const { desde, hasta } = req.query;
    let desdeDate = null;
    ...
    try {
        ...
        const tienda = tiendaRes.recordset[0];
        const result = await pool.request()
            .input('tiendaId', req.user.tiendaId)
            .input('desde', desdeDate)
            .input('hasta', hastaDate)
        ...
        doc.pipe(res);
        const fmtNum = (n) => n.toLocaleString('es-CO', { maximumFractionDigits: 0 });
        const fmtFecha = (d) => {
            if (!d)
                return '';
            const dt = new Date(d);
...
app.get('/ventas/:id/devoluciones', authMiddleware, async (req, res) => {
    if (!req.user) {
        return res.status(401).json({ message: 'No autorizado' });
    }
    const { id } = req.params;
...
app.post('/ventas/:id/devolucion', authMiddleware, async (req, res) => {
    if (!req.user) {
        return res.status(401).json({ message: 'No autorizado' });
    }
    const { id } = req.params;
    ...
    try {
        ...
        await tx.begin();
        try {
            const detalleRes = await new db_js_1.sql.Request(tx)
                .input('ventaId', ventaId)
                .input('tiendaId', req.user.tiendaId)
                .query(`
          ...
          `);
            ...
            for (const r of devolucionesPrev.recordset) {
...
app.post('/movimientos-inventario', authMiddleware, async (req, res) => {
    if (!req.user) {
        return res.status(401).json({ message: 'No autorizado' });
    }
    const { tipoMovimiento, productoId, varianteId, cantidad, motivo } = req.body;
...
app.put('/ventas/:id', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (estado === 'Cancelado') {
            ...
            await tx.begin();
            try {
                const estadoActual = await new db_js_1.sql.Request(tx)
                    .input('ventaId', ventaId)
                    .input('tiendaId', req.user.tiendaId)
                    .query(`SELECT Estado FROM Ventas WHERE Id = @ventaId AND Tienda_Id = @tiendaId`);
                const currentEstado = estadoActual.recordset[0]?.Estado;
                if (currentEstado !== 'Cancelado') {
                    const detalleRes = await new db_js_1.sql.Request(tx)
                        .input('ventaId', ventaId)
                        .query(`SELECT Producto_Id, Variante_Id, Cantidad FROM Venta_Detalle WHERE Venta_Id = @ventaId`);
                    const items = detalleRes.recordset;
                    for (const it of items) {
                        await new db_js_1.sql.Request(tx)
                            .input('productoId', db_js_1.sql.Int, it.Producto_Id)
                            .input('cantidad', db_js_1.sql.Int, it.Cantidad)
                            .query(`UPDATE Productos SET StockActual = ISNULL(StockActual, 0) + @cantidad WHERE Id = @productoId`);
                        if (it.Variante_Id != null) {
                            await new db_js_1.sql.Request(tx)
                                .input('varianteId', db_js_1.sql.Int, it.Variante_Id)
                                .input('cantidad', db_js_1.sql.Int, it.Cantidad)
                                .query(`UPDATE Producto_Variaciones SET StockActual = ISNULL(StockActual, 0) + @cantidad WHERE Id = @varianteId`);
                        }
                        await new db_js_1.sql.Request(tx)
                            .input('tiendaId', db_js_1.sql.UniqueIdentifier, req.user.tiendaId)
                            .input('productoId', db_js_1.sql.Int, it.Producto_Id)
                            .input('variacionId', db_js_1.sql.Int, it.Variante_Id ?? null)
                            .input('cantidad', db_js_1.sql.Int, it.Cantidad)
                            .input('motivo', db_js_1.sql.NVarChar, `Anulación venta #${ventaId}`)
                            .query(`
                  INSERT INTO Movimientos_Inventario (Tienda_Id, Producto_Id, Variacion_Id, TipoMovimiento, Cantidad, Motivo, Fecha)
